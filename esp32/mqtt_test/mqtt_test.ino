...
void setup() {
  ...
  Serial.println("  Команды: vehicles/" + String(mqtt_client_id) + "/commands");
  Serial.println("");
...
void sendTelemetryMessage() {
  ...
  if (client.publish(telemetry_topic, payload.c_str(), false)) { // QoS 0, не retain
    Serial.println("📊 [" + String(messageCount) + "] ТЕЛЕМЕТРИЯ → " + String(telemetry_topic));
    Serial.println("    📦 " + payload);
    Serial.println("    📍 " + String(current_lat, 5) + ", " + String(current_lng, 5) + " | " + String(speed) + " км/ч | " + status_str);
  } else {
...
void sendHeartbeatMessage() {
  ...
  if (client.publish(heartbeat_topic, payload.c_str(), false)) {
    Serial.println("💓 HEARTBEAT → " + String(heartbeat_topic) + " (uptime: " + String(millis()/1000) + "s)");
  } else {
...
void sendStatusMessage(String status) {
  ...
  if (client.publish(status_topic, payload.c_str(), true)) { // Retain = true для статуса
    Serial.println("📡 СТАТУС → " + String(status_topic) + ": " + status);
  } else {
