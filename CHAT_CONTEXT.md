...
### Найденные проблемы:
1. Git репозиторий НЕ настроен в `/var/www/mapmon/`
2. Ошибки SSR в pages/history.vue (строка 86)
3. MQTT timeout ошибки от публичного брокера
4. База данных не найдена в server-backup/
...
Последняя задача: исправлены утечки памяти, нужно обновить VPS сервер.
```
